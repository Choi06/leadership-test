...
# --- 페이지 설정 ---
st.set_page_config(page_title="한국형 기업가 리더십 검사", layout="wide")

# --- 데이터 저장소 (세션 상태 이용) ---
if 'db' not in st.session_state:
    st.session_state.db = pd.DataFrame(columns=['ID', 'Type', 'Time', 'Pioneer', 'Architect', 'Harmonizer', 'Steward'])

# --- 리더 데이터 정의 ---
leaders = {
    "Pioneer": {"name": "정주영 (현대)", "desc": "불도저형: '해봤어?' 정신으로 불가능을 가능케 하는 실행력의 화신"},
    "Architect": {"name": "이병철 (삼성)", "desc": "설계자형: '인재제일'과 철저한 분석으로 시스템을 구축하는 전략가"},
    "Harmonizer": {"name": "구인회 (LG)", "desc": "화합형: '인화'를 바탕으로 사람을 믿고 함께 성장하는 포용의 리더"},
    "Steward": {"name": "박태준 (포스코)", "desc": "원칙주의형: '제철보국'의 사명감으로 완벽을 추구하는 강직한 리더"}
}
...
# --- 메인 로직 ---
st.title("🇰🇷 한국형 기업가 리더십 유형 검사 (20문항)")

# 1. 로그인 (ID 입력)
if 'user_id' not in st.session_state or st.session_state.user_id == "":
    st.subheader("검사를 위해 아이디를 입력해주세요.")
    uid = st.text_input("ID 입력")
    if st.button("시작하기"):
        if uid:
            st.session_state.user_id = uid
            st.rerun()
        else:
            st.error("ID를 입력해야 합니다.")
    st.stop()

# 2. 사이드바 메뉴
menu = st.sidebar.selectbox("메뉴", ["리더십 검사", "관리자 대시보드"])

if menu == "리더십 검사":
    st.write(f"반갑습니다, **{st.session_state.user_id}** 님!")
    with st.form("survey"):
        user_answers = []
        ...
        if submitted:
            ...
            for i, ans in enumerate(user_answers):
                if ans == questions[i]['a']: s["Pioneer"] += 1
                elif ans == questions[i]['b']: s["Architect"] += 1
                elif ans == questions[i]['c']: s["Harmonizer"] += 1
                elif ans == questions[i]['d']: s["Steward"] += 1
            
            res_type = max(s, key=s.get)
            
            # DB 저장
            new_record = {
                'ID': st.session_state.user_id, 'Type': res_type, 
                'Time': datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                'Pioneer': s['Pioneer'], 'Architect': s['Architect'],
                'Harmonizer': s['Harmonizer'], 'Steward': s['Steward']
            }
            st.session_state.db = pd.concat([st.session_state.db, pd.DataFrame([new_record])], ignore_index=True)
            
            # 결과 화면 출력
            st.markdown("---")
            st.header(f"당신의 리더십 유형은: **{leaders[res_type]['name']}**")
            st.subheader(leaders[res_type]['desc'])
            
            # 차트
            fig = go.Figure()
            fig.add_trace(go.Scatterpolar(
                r=[s['Pioneer'], s['Architect'], s['Harmonizer'], s['Steward']],
    ...
    st.subheader("📊 응답 통계 확인")
    pw = st.text_input("관리자 암호", type="password")
    if pw == "0000":
        if not st.session_state.db.empty:
            st.write(f"총 응답수: {len(st.session_state.db)}건")
            st.dataframe(st.session_state.db[['ID', 'Type', 'Time']])
            
            # 비율 차트
            type_counts = st.session_state.db['Type'].value_counts()
            fig_pie = px.pie(values=type_counts.values, names=[leaders[t]['name'] for t in type_counts.index], title="유형별 분포")
