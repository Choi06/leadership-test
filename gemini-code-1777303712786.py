...
leaders_info = {
    "Pioneer": {"name": "정주영 (현대그룹)", "motto": "“이봐, 해봤어?”", "bio": "불가능을 가능으로 바꾼 한국 경제 실행력의 상징.", "case": "유조선 물막이 공법과 500원권 거북선 지폐 차관 유치.", "hashtags": ["#실행", "#불도저", "#현장"], "img": "https://upload.wikimedia.org/wikipedia/ko/c/c1/Chung_Ju-yung.jpg"},
    "Architect": {"name": "이병철 (삼성그룹)", "motto": "“인재제일(人才第一).”", "bio": "치밀한 분석과 시스템으로 삼성을 글로벌 기업으로 키운 전략가.", "case": "73세의 나이에 단행한 반도체 투자 결정으로 IT 강국의 기틀 마련.", "hashtags": ["#시스템", "#데이터", "#전략"], "img": "https://upload.wikimedia.org/wikipedia/commons/e/e0/Lee_Byung-chul.jpg"},
    "Harmonizer": {"name": "구인회 (LG그룹)", "motto": "“인화(人和)가 제일이다.”", "bio": "사람을 믿는 경영으로 신뢰와 상생의 기업 문화를 닦은 리더.", "case": "동업자와의 신뢰를 유지하며 전자 산업의 불모지를 개척함.", "hashtags": ["#인화", "#상생", "#신뢰"], "img": "https://upload.wikimedia.org/wikipedia/ko/2/29/Koo_In-hwoi.jpg"},
    "Steward": {"name": "박태준 (포스코)", "motto": "“제철보국(製鐵報國).”", "bio": "무결점 완벽주의로 철강 산업의 신화를 쓴 강직한 사명가.", "case": "부실 공사 구조물 폭파와 사심 없는 포항제철 건설 과정.", "hashtags": ["#사명", "#무결점", "#원칙"], "img": "https://upload.wikimedia.org/wikipedia/ko/d/d4/Park_Tae-joon.jpg"}
}
...
if st.session_state.page == 'home':
    ...
    st.markdown(f"""
        <div class="hero-container">
            <img src="https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?q=80&w=2000" class="hero-image">
            <div class="hero-text">
                <h1 style='font-weight: 800; font-size: 2.8rem; margin-bottom: 0;'>K-Leadership Index</h1>
                <p style='font-size: 1.2rem; opacity: 0.9;'>대한민국을 일궈낸 거인들, 당신의 리더십 모델은?</p>
            </div>
        </div>
    ...
    st.markdown(f"<h3 style='font-weight: 700;'>📋 리더십 성향 조사 ({step}/2)</h3>", unsafe_allow_html=True)
    ...
    st.markdown("<h2 style='font-weight: 800;'>🏆 분석 리포트</h2>", unsafe_allow_html=True)
    ...
    st.markdown(f"""
        <div class="encyclopedia-box">
            <h2 style='color: #111111; margin-bottom: 0;'>{leader['name']} 스타일</h2>
            <p style='font-style: italic; color: #555555; font-size: 1.2rem; margin-top: 5px;'>{leader['motto']}</p>
            <hr style='border: 0.5px solid #111111; margin: 15px 0;'>
            <p style='line-height: 1.7;'><b>분석:</b> {leader['bio']}</p>
        </div>
    """, unsafe_allow_html=True)
    ...
    fig = go.Figure(data=go.Scatterpolar(
        ...
        theta=['개척', '설계', '화합', '원칙'],
        fill='toself', line=dict(color='#111111')
    ))
    fig.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, 10])), showlegend=False)
    st.plotly_chart(fig, use_container_width=True)
    
    if st.button("처음으로"): go_to('home')

elif st.session_state.page == 'dictionary':
    st.markdown("<h2 style='font-weight: 800;'>📚 인물 사전</h2>", unsafe_allow_html=True)
    cols = st.columns(2)
    keys = ["Pioneer", "Architect", "Harmonizer", "Steward"]
    for i, k in enumerate(keys):
        if cols[i%2].button(leaders_info[k]['name'].split(" ")[0]):
            st.session_state.selected_leader = k
            
    info = leaders_info[st.session_state.selected_leader]
    st.markdown(f"""
        <div class="encyclopedia-box">
            <h3 style='margin-bottom: 0;'>{info['name']}</h3>
            <p style='color: #555555; font-style: italic;'>{info['motto']}</p>
            <img src="{info['img']}" style='width: 100%; max-width: 400px; border-radius: 8px; margin: 15px 0; display: block; margin-left: auto; margin-right: auto;'>
            <p style='line-height: 1.7;'><b>📍 약력:</b> {info['bio']}</p>
            <p style='line-height: 1.7;'><b>💡 주요사례:</b> {info['case']}</p>
        </div>
    """, unsafe_allow_html=True)
