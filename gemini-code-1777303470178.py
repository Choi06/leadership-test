...
st.markdown("""
    ...
    html, body, [class*="st-"] {
        ...
        color: #111111 !important; /* 본문 글자 검정색 통일 */
    ...
    .hero-image {
        ...
        object-fit: cover;
        border-radius: 15px;
        filter: brightness(0.6); /* 사진 위 글자가 잘 보이게 어둡게 조절 */
    }
    .hero-text {
        position: absolute;
        top: 50%;
...
leaders_info = {
    "Pioneer": {"name": "정주영 (현대그룹)", "motto": "“이봐, 해봤어?”", "bio": "가난한 농군의 아들로 태어나 현대그룹을 일궈낸 실행력의 상징.", "case": "서산 간척지 물막이 공사와 거북선 지폐 차관 유치.", "hashtags": ["#실행", "#불도저", "#현장"], "img": "https://upload.wikimedia.org/wikipedia/ko/c/c1/Chung_Ju-yung.jpg"},
    "Architect": {"name": "이병철 (삼성그룹)", "motto": "“인재제일(人才第一).”", "bio": "치밀한 전략과 시스템으로 삼성을 글로벌 기업으로 키운 전략가.", "case": "73세에 단행한 반도체 투자 결정으로 IT 강국의 기틀 마련.", "hashtags": ["#시스템", "#데이터", "#전략"], "img": "https://upload.wikimedia.org/wikipedia/commons/e/e0/Lee_Byung-chul.jpg"},
    "Harmonizer": {"name": "구인회 (LG그룹)", "motto": "“인화(人和)가 제일이다.”", "bio": "사람을 믿는 경영으로 화학과 전자 산업의 기틀을 닦은 리더.", "case": "동업자와의 신뢰를 유지하며 안정적인 기업 문화를 정착시킴.", "hashtags": ["#인화", "#상생", "#신의"], "img": "https://upload.wikimedia.org/wikipedia/ko/2/29/Koo_In-hwoi.jpg"},
    "Steward": {"name": "박태준 (포스코)", "motto": "“제철보국(製鐵報國).”", "bio": "무결점 완벽주의로 한국 철강 산업의 신화를 쓴 강직한 사명가.", "case": "부실 공사 구조물 폭파와 헌신적인 제철소 건설 과정.", "hashtags": ["#사명", "#무결점", "#원칙"], "img": "https://upload.wikimedia.org/wikipedia/ko/d/d4/Park_Tae-joon.jpg"}
}
...
if st.session_state.page == 'home':
    ...
    st.markdown(f"""
        <div class="hero-container">
            ...
            <div class="hero-text">
                <h1 style='color: white; font-weight: 800; font-size: 2.5rem;'>K-Leadership Index</h1>
                <p style='color: white; font-size: 1.1rem; opacity: 0.9;'>대한민국을 만든 거인들, 당신의 리더십 모델은 누구입니까?</p>
            </div>
        </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown(f"### 📋 리더십 성향 조사지 ({step}/2)")
    ...
    if step == 1:
        if st.button("다음 페이지로 (11~20번)"): st.session_state.survey_step = 2; st.rerun()
    else:
        if st.button("최종 결과 확인하기"): go_to('result')
    ...
    st.markdown(f"""
        <div class="encyclopedia-box">
            <h2 style='color: #111111; margin-bottom: 5px;'>{leader['name']} 스타일</h2>
            <p style='font-style: italic; color: #555555; font-size: 1.2rem;'>{leader['motto']}</p>
            <hr style='border: 0.5px solid #111111;'>
            <p><b>분석내용:</b> {leader['bio']}</p>
        </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("## 📚 인물 사전")
    cols = st.columns(2) # 모바일 대응을 위해 2열로 배치
    ...
    st.markdown(f"""
        <div class="encyclopedia-box">
            ...
            <p style='color: #555555;'>{info['motto']}</p>
            <img src="{info['img']}" style='width: 100%; border-radius: 5px; margin: 10px 0;'>
            <p><b>약력:</b> {info['bio']}</p>
            <p><b>사례:</b> {info['case']}</p>
        </div>
    """, unsafe_allow_html=True)
